...
['#singleDoorLQty','#singleDoorRQty','#parentChildDoorLQty','#parentChildDoorRQty','#doubleDoorLQty','#doubleDoorRQty'].forEach(id=>$(id).addEventListener('input',updatePlacementUI));
['#singleDoorWidth','#singleDoorHeight','#parentChildDoorWidth','#parentChildDoorHeight','#doubleDoorWidth','#doubleDoorHeight','#totalWidth','#endWidth','#insertion','#minPanel'].forEach(id=>$(id).addEventListener('input',updatePlacementUI));
...
function renderDoorSymbols(doors,placements=[]){
  ...
  $('#doorSymbols').innerHTML=active.length?active.map(({d,hand,qty,span,sleeve})=>`<div class="door-symbol-card">${doorSvg(hand,d.key)}<div><b>${d.label} ${hand}</b><span>${span?`${span}スパン目・`:''}${qty}${d.unit}・W${d.width}×H${d.height}${sleeve?`・袖W${sleeve}`:''}</span></div></div>`).join(''):'<span class="door-card-empty">ドア選択時に、平面図形式のL／R記号を表示します。</span>';
}
...
function render(result){
  ...
  $('#partsGrid').innerHTML=items.map(x=>`<div class="part ${x[3]}"><span>${x[0]}</span><strong>${x[1]}</strong><small>${x[2]}</small></div>`).join('')+'<div class="door-result-note">L／Rは平面図上の開き勝手です。指定スパンのパネルをドア＋袖パネルへ置換しています。ドア詳細部材は仮マスタです。</div>';
...
async function exportExcel(){
  ...
  const rows=[['製品',`${r.product}（${r.product_name}）`],['形状',r.shape],['総幅',r.dimensions.total_width],['割付対象幅',r.dimensions.allocation_width],['CH',r.input.ch],['END',r.counts.end],['2WAY',r.counts.two_way],['3WAY',r.counts.three_way],['W1200',r.counts.panel_1200],['W900',r.counts.panel_900],['調整パネル',r.counts.adjustment],['調整幅',r.dimensions.adjustment_width],
    ...
    ...r.placements.flatMap((x,i)=>[[`ドア配置 ${i+1}`,`${x.span}スパン目 ${x.label} ${x.hand}`],[`袖パネル ${i+1}`,x.sleeve_width?`W${x.sleeve_width}×H${x.sleeve_height}`:'なし'],[`欄間パネル ${i+1}`,x.transom_height?`W${x.transom_width}×H${x.transom_height}`:'なし']]),
