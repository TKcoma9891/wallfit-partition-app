...
function render(result){
  ...
  $('#partsGrid').innerHTML=items.map(x=>`<div class="part ${x[3]}"><span>${x[0]}</span><strong>${x[1]}</strong><small>${x[2]}</small></div>`).join('')+'<div class="door-result-note">L／Rは平面図上の開き勝手です。指定スパンをドアへ置換し、袖幅と隣接パネルをW1200以内の1枚にまとめられる場合は袖を自動統合します。ドア詳細部材は仮マスタです。</div>';
